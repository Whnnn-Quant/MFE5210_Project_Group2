...
Strategy_Mom_ma_result_path = "/Users/wanghenan/港中深金工/Courses/AL_Trade/Project/results/Mom_double_ma.csv"
Strategy_Multi_factor_result_path = "/Users/wanghenan/港中深金工/Courses/AL_Trade/Project/results/Multi_factor.csv"
...
def plot_strategy_performance(returns):
    ...
    fig.suptitle('Strategy Performance Analysis', fontsize=14, y=0.97)
...
df_multi_factor = pd.read_csv(Strategy_Multi_factor_result_path,index_col=0)
df_multi_factor.index = pd.to_datetime(df_multi_factor.index)

df_mom_double_ma = pd.read_csv(Strategy_Mom_ma_result_path,index_col=0)
df_mom_double_ma.index = pd.to_datetime(df_mom_double_ma.index)

df_combine = pd.concat([df_multi_factor,df_mom_double_ma],axis=1)
df_combine['daily_ret'] = df_combine.mean(axis=1)

plot_strategy_performance(df_combine['daily_ret'])